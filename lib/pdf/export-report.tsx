...
function normalizeTableArray(
  ...
  return input
    ...
    .filter((row): row is Record<string, Primitive> => row !== null);
}
...
function normalizeScenarioArray(
  ...
  return input
    .map((item) => {
      ...
      const description = asNullableString(row.description ?? row.body ?? row.text);
    ...
    .filter(
      (item): item is { name: string; value?: string; description?: string } =>
        item !== null,
    );
}
...
function normalizeSentimentArray(
  ...
  return input
    ...
    .filter(
      (item): item is { label: string; score: string; description?: string } =>
        item !== null,
    );
}

function normalizeImage(input: unknown): { src?: string | null } | null {
...
function normalizeAppendixImages(
  ...
  return input
    .map((item): { title?: string; subtitle?: string; src: string } | null => {
      if (!item || typeof item !== 'object') return null;
    ...
    .filter(
      (item): item is { title?: string; subtitle?: string; src: string } =>
        item !== null && typeof item.src === 'string' && item.src.length > 0,
    );
}
...
export function mapReportToPdfData(
  ...
  return {
    ...
    growthCatalysts: normalizeBulletArray(payload.growthCatalysts ?? payload.catalysts),
    ...
    dashboardTable: normalizeTableArray(payload.dashboardTable ?? payload.tables?.dashboard),
    businessSegmentsTable: normalizeTableArray(
      payload.businessSegmentsTable ?? payload.tables?.businessSegments,
    ),
    forecastTable: normalizeTableArray(payload.forecastTable ?? payload.tables?.forecast),
    peerTable: normalizeTableArray(payload.peerTable ?? payload.tables?.peers),
    modelTable: normalizeTableArray(payload.modelTable ?? payload.tables?.model),
    valuationTable: normalizeTableArray(payload.valuationTable ?? payload.tables?.valuation),
    ...
    dashboardImage: normalizeImage(payload.dashboardImage ?? payload.images?.dashboard),
    thesisImage: normalizeImage(payload.thesisImage ?? payload.images?.thesis),
    technicalImage: normalizeImage(payload.technicalImage ?? payload.images?.technical),
    tamImage: normalizeImage(payload.tamImage ?? payload.images?.tam),
    valuationImage: normalizeImage(payload.valuationImage ?? payload.images?.valuation),
    conclusionImage: normalizeImage(payload.conclusionImage ?? payload.images?.conclusion),
