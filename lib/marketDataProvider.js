...
async function getPeerTickers(ticker) {
  const url = `${process.env.MARKET_DATA_BASE_URL}/stock_peers?symbol=${encodeURIComponent(
    ticker
  )}&apikey=${encodeURIComponent(process.env.MARKET_DATA_API_KEY || "")}`;
  ...
  if (Array.isArray(data?.peersList)) {
    return data.peersList.slice(0, 6);
  }

  if (Array.isArray(data?.peers)) {
    return data.peers.slice(0, 6);
  }
